...
const sendLoginOtpEmail = async ({ toEmail, userName, otp }) => {
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; border: 1px solid #e1e1e1; padding: 20px;">
      <h2 style="color: #1da1f2;">Verify Your Login</h2>
      <p>Hi ${userName},</p>
      <p>We detected a login attempt from Google Chrome. Please enter the code below to complete your login. This code is valid for 10 minutes.</p>
      <p style="margin: 20px 0; text-align: center;">
        <span style="display: inline-block; background: #f5f8fa; border: 1px dashed #1da1f2; padding: 10px 24px; font-size: 24px; font-weight: bold; letter-spacing: 4px; border-radius: 6px;">${otp}</span>
      </p>
...
module.exports = { sendInvoiceEmail, sendNewPasswordEmail, sendLoginOtpEmail };
